...
# Definitions
INCLUDES = ['btBulletDynamicsCommon.h',os.path.join('BulletDynamics','ConstraintSolver','btTypedConstraint.h'), os.path.join('BulletCollision', 'CollisionShapes', 'btHeightfieldTerrainShape.h'), os.path.join('BulletCollision', 'CollisionDispatch', 'btGhostObject.h'), os.path.join('BulletDynamics', 'Character', 'btKinematicCharacterController.h'), os.path.join('BulletSoftBody', 'btSoftBody.h'), os.path.join('BulletSoftBody', 'btSoftRigidDynamicsWorld.h'), os.path.join('BulletSoftBody', 'btDefaultSoftBodySolver.h'), os.path.join('BulletSoftBody', 'btSoftBodyRigidBodyCollisionConfiguration.h'), os.path.join('BulletSoftBody', 'btSoftBodyHelpers.h')]


'''

-------	Include all headers in bullet/src ------

files=set()
INCLUDES=[]
os.chdir('bullet/src')
for (dirpath,dirname,file) in os.walk(os.getcwd()):
	for i in file:
		if i.endswith('.h') and i not in files and 'sse' not in dirpath:
			files.add(i)
			INCLUDES.append(os.path.join(dirpath,i))
...
#emcc_args += '-s EXPORT_NAME="Hello" -s MODULARIZE=1'.split(' ')

#	TARGET CHANGED TO hello.js
target = 'hello.js' if not wasm else 'hello.wasm.js'
...
try:
  ...
  print ' '.join(args)
	...
	#		LINE ADDED:	GENERATE bitcode file FOR HELLO.CPP
  emscripten.Building.emcc('hello.cpp', args, 'hello.bc')
  ...
  if cmake_build:
	...
	#	LINE MODIFIED:	LINK hello.bc PREVIOUSLY GENERATED WITH BULLET LIBRARIES
  emscripten.Building.link(['glue.bc','hello.bc'] + bullet_libs, 'hellobullet.bc')
  assert os.path.exists('hellobullet.bc')

  stage('emcc: ' + ' '.join(emcc_args))

  temp = os.path.join('..', '..', 'builds', target)
  print "PATH HERE: ",temp, os.getcwd()
  emscripten.Building.emcc('hellobullet.bc', emcc_args + ['--js-transform', 'python %s' % os.path.join('..', '..', 'bundle.py')],
                           temp)
